...
class ServerInspector:
    ...
    def __init__(self):
        self.specs = {
            "collection_info": {
                ...
                "method": "local",  # Can be 'local' or 'ssh'
            },
            "server": {},
            "storage": {},
            "network": {},
    ...
    def run(self) -> dict[str, Any]:
        ...
        print_header("HOME SERVER HARDWARE INSPECTOR")
...
def main() -> None:
    ...
    parser.add_argument(
        "--output", "-o", default="server-specs.json", help="Output JSON file (default: server-specs.json)"
    )
    ...
    try:
        ...
        inspector.run()
        inspector.save_json(args.output)
        inspector.print_summary()

        print()
        print(f"{Colors.BOLD}Next Steps:{Colors.END}")
        print(f"  1. Review {args.output} (optional)")
        print("  2. Unmount and shutdown:")
        print("     cd / && sync && umount /mnt/persistence")
        print("     poweroff")
        print("  3. Remove USB and insert into admin PC")
        print("  4. Retrieve JSON from USB using live-usb-helper retrieve command")
        print()
